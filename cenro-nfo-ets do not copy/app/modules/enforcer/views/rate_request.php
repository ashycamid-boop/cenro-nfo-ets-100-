...
if (!function_exists('enforcer_rate_signature_proxy_url')) {
  function enforcer_rate_signature_proxy_url($path)
  {
...
<body class="admin-dashboard-page enforcement-service-requests-page" data-rating-finalized="<?php echo $rating_finalized ? '1' : '0'; ?>">
  <div class="layout">
    <button class="mobile-sidebar-backdrop" type="button" aria-label="Close sidebar"></button>
    <!-- Sidebar -->
    <nav class="sidebar" id="enforcerRateRequestSidebar" role="navigation" aria-label="Main sidebar">
      <div class="sidebar-logo">
        <img src="../../../../public/assets/images/denr-logo.png" alt="DENR Logo">
        <span>CENRO</span>
      </div>
      <div class="sidebar-role"><?php echo htmlspecialchars((string)($_SESSION['role'] ?? 'Enforcer'), ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <div class="main">
      <div class="topbar">
        <div class="topbar-card">
          <button class="mobile-nav-toggle" type="button" aria-label="Open navigation" aria-expanded="false" aria-controls="enforcerRateRequestSidebar">
...
require_once dirname(__DIR__, 3) . '/config/app.php';
          ...
          $requester_sig_url = !empty($request['requester_signature_path']) ? enforcer_rate_signature_proxy_url($request['requester_signature_path']) : '';
          $auth1_sig_url = !empty($request['auth1_signature_path']) ? enforcer_rate_signature_proxy_url($request['auth1_signature_path']) : '';
          $auth2_sig_url = !empty($request['auth2_signature_path']) ? enforcer_rate_signature_proxy_url($request['auth2_signature_path']) : '';
          $ack_sig_url = !empty($request['ack_signature_path'])
            ? enforcer_rate_signature_proxy_url($request['ack_signature_path'])
            : '';
...
require_once dirname(__DIR__, 3) . '/config/app.php';
                        ...
                        <td style="border-bottom: 1px solid black; padding: 6px; text-align:center; vertical-align:middle;">
                          <?php if (!empty($action['action_signature_path'])): ?>
                            <img src="<?php echo htmlspecialchars(enforcer_rate_signature_proxy_url($action['action_signature_path'])); ?>" alt="Action Signature" style="max-height:48px; max-width:100%;" />
                          <?php else: ?>
...
require_once dirname(__DIR__, 3) . '/config/app.php';
  ...
  <script src="../../../../public/assets/js/enforcer/request_details.js?v=20260520-ack-signature-required"></script>
