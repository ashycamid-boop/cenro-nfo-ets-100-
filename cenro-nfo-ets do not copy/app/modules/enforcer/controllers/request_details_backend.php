...
if (!function_exists('enforcer_signature_proxy_url')) {
  function enforcer_signature_proxy_url($path)
  {
    ...
    return ($projectBase !== '' ? $projectBase : '') . '/app/modules/enforcer/views/signature_image.php?path=' . rawurlencode(str_replace('\\', '/', ltrim($path, '/')));
  }
}
...
$requester_sig_url = !empty($request['requester_signature_path'])
  ? enforcer_signature_proxy_url($request['requester_signature_path'])
  : '';
$auth1_sig_url = !empty($request['auth1_signature_path'])
  ? enforcer_signature_proxy_url($request['auth1_signature_path'])
  : '';
$auth2_sig_url = !empty($request['auth2_signature_path'])
  ? enforcer_signature_proxy_url($request['auth2_signature_path'])
  : '';
$ack_sig_url = !empty($request['ack_signature_path'])
  ? enforcer_signature_proxy_url($request['ack_signature_path'])
  : '';
