(function () {
  ...
  const hasExistingPreview = !!(ackSigPreview && ackSigPreview.getAttribute('src') && ackSigPreview.getAttribute('src').trim() !== '');
  ...
  if (ackBox) {
    ackBox.addEventListener('click', function () {
      if (ackBox.dataset.saved === '1' || !ackModal) {
        return;
      }
      try {
        resizeCanvas();
      ...
      if (ackHidden && ackHidden.value) {
        try {
          signaturePad.fromDataURL(ackHidden.value);
        } catch (e) {}
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', function () {
      ...
      if (signaturePad.isEmpty()) {
        ...
        if (ackPlaceholder) {
          ackPlaceholder.style.display = 'inline';
        }
        ackModal.hide();
        return;
      }
      ...
      if (dataURL) {
        ...
        if (ackPlaceholder) {
          ackPlaceholder.style.display = 'none';
        }
        if (ackSaveBtn) {
          ackSaveBtn.disabled = false;
        }
      }
      ackModal.hide();
    });
  ...
  if (ackSaveBtn) {
    if (ratingFinalized) {
      ...
      if (savedLabel) {
        savedLabel.style.display = 'inline-block';
      }
      if (ackBox) {
    ...
    if (completedInput) {
      completedInput.addEventListener('change', enableSave);
    }
    ...
    ackSaveBtn.addEventListener('click', async function () {
      ...
      try {
        ...
        ackSaveBtn.classList.add('btn-success');
        if (ackSavedLabel) {
          ackSavedLabel.style.display = 'inline-block';
        }

        const dtEl = document.getElementById('ack_datetime_display');
        if (dtEl) {
          dtEl.textContent = formatDate12(new Date());
        }
  ...
  try {
    if ((ackHidden && ackHidden.value) || hasExistingPreview) {
      if (ackSigPreview) {
        ackSigPreview.style.display = 'block';
      }
      if (ackPlaceholder) {
