...
try {
  ...
  if ($dbPath !== null && in_array('ack_signature_path', $cols)) {
    ...
    if (in_array('acknowledged_by', $cols)) { $sets[] = 'acknowledged_by = ?'; $params[] = $_SESSION['full_name'] ?? 'Enforcement Officer'; }
  }
  ...
  $out = ['ok' => true];
  if ($dbPath) $out['path'] = enforcement_officer_signature_proxy_url($dbPath);
